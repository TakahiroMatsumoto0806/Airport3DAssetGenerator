...
**Step 3（3D 生成）は本プロジェクトの対象範囲外です。** 画像 → 3D メッシュ生成は別 PC（x86_64 + RTX 5090）で別プロジェクトとして実施するため、本リポジトリのセットアップ・ドキュメントは Step 3 を**含みません**。生成された GLB ファイルを `outputs/meshes_raw/` に配置した状態から、本パイプラインのメッシュ QA 以降（メッシュ QA、マルチビュー 3D QA、Step 4、Step 5）を実行してください。
...
**Step 3（3D 生成）は本プロジェクトの対象範囲外です。** 画像 → 3D メッシュ生成は別 PC・別プロジェクトで実施します。本プロジェクトでは、受け取った GLB ファイルを `outputs/meshes_raw/` に配置した状態から後続ステップを実行します。
...
├── meshes_approved/
│   ├── *.glb                             # メッシュ QA + マルチビュー 3D QA 合格メッシュ
│   └── mesh_vlm_qa_results.json          # マルチビュー 3D QA スコア記録
├── assets_final/
...
│   ├── physics_report.html               # 物理プロパティ付与結果（HTML）
│   ├── pass_rate_report.html             # カテゴリ別合格率レポート（HTML）
│   └── pass_rate_report.json             # カテゴリ別合格率サマリー（JSON）
└── logs/                                 # 実行ログ
```

各アセットには provenance（pass / review / reject）が JSON で記録されます。
...
| カテゴリ固有の短い先頭フレーズを追加（CLIP 77 トークン枠の先頭に必ず届く） | `category_clip_prefix.<カテゴリ>`（例: ハンドル格納・バッグ閉鎖など画像 QA で弾かれやすい指示を優先） |
...
各値は YAML の定義値を基準に、**パイプライン実行時に自動でランダム化されてアセットごとに異なる値が割り当てられます**（学習データの多様性確保のため。ランダム化機能はすでに実装・デフォルト有効です）。ランダム化を止めたい場合は該当材質の `randomize_range: 0.0` に設定してください。

| 物理量 | 決定方法 | 実装 |
|--------|---------|------|
| **密度** `density_kg_m3` | YAML 指定値 ± `randomize_range` の一様分布からサンプリング | [physics_processor.py:206](src/physics_processor.py#L206) |
...
| **静止摩擦** `static_friction` | YAML 指定値 ± `randomize_range` の一様分布からサンプリング | [physics_processor.py:207](src/physics_processor.py#L207) |
| **動摩擦** `dynamic_friction` | YAML 指定値 ± `randomize_range` の一様分布からサンプリング | [physics_processor.py:208](src/physics_processor.py#L208) |
| **反発係数** `restitution` | YAML 指定値 ± `randomize_range` の一様分布からサンプリング | [physics_processor.py:209](src/physics_processor.py#L209) |

**ランダム化の仕組み**

```text
サンプリング値 ∈ [ value − value × rr,  value + value × rr ]   # 一様分布
```

- `value` は YAML で指定した基準値（例: `density_kg_m3: 1200`）
- `rr` は `materials.<name>.randomize_range`（未指定時は `defaults.randomize_range = 0.15`）
- 例: `density_kg_m3 = 1200`, `rr = 0.15` の場合、`[1020, 1380]` の一様分布からサンプリングされる
- **`rr = 0.0` にするとランダム化は実質無効となり、基準値がそのまま使われる**

> [!IMPORTANT]
> 乱数は `PhysicsProcessor(seed=42)` で初期化された `random.Random` から供給されます。**seed を固定している限り、ランダム化後の値は完全に再現されます**（[physics_processor.py:58](src/physics_processor.py#L58)）。
...
`measure_pass_rates.py` や `--no-resume` 実行は既存の `outputs/` を上書きするため、重要な出力は事前にバックアップしてください。
...
**Step 3（3D 生成）は本プロジェクトの対象範囲外です。** 別 PC・別プロジェクトで実施し、生成された GLB を `outputs/meshes_raw/` に配置した状態から本プロジェクトのメッシュ QA 以降を実行してください。
